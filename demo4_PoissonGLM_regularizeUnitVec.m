% demo4_PoissonGLM_regularizeUnitVec.m
%
% Regularize using a penalty on the normalized parameter vector, then see if this can be used for unbiased recovery of the nonlinearity power p.
...
% Set log-prior function
Dnrm = speye(nw+1)/nw;  Dnrm(nw+1,nw+1) = 0;  % matrix for normalizing weights
logPriFun = @(prs,C)(logprior_normalizedGaussian(prs,C,Dnrm)); % set log-prior function
...
for jj = 1:nlam
    ...
    lossfun = @(prs)neglogposterior_nonGaussianprior(prs,negLtrainfun,logPriFun,Cinv);
...
CinvHat = lambda*L;  % set fixed regularization matrix
logPriFun = @(prs)(logprior_normalizedGaussian(prs,CinvHat,Dnrm)); % set log-prior function
...
for jj = 1:npgrid
    ...
    lossfun = @(prs)neglogposterior_nonGaussianprior(prs,negLtrainfun,logPriFun);
...
xlabel('power p'); ylabel('log-likeihood');
...
subplot(224);
ttw = 1:nw;
plot(ttw,wts./norm(wts), ttw,w_MAP1./norm(w_MAP1), ttw,w_MAPfittedp./norm(w_MAPfittedp));
axis tight;
xlabel('time bin'); ylabel('coefficient');
title('normalized true and inferred weights');
legend('true','MAP(p=1)','MAP (fitted p)');
