% Unit test to ensure that mkPrecision_AR1prior_circ_nonunif produces a precision matrix with
% the correct inverse
...
L = mkPrecision_AR1prior_circ(tau,rho,nw);
C = inv(L);  % invert it
...
Lreduced2 = mkPrecision_AR1prior_circ_nonunif(tau,rho,rowindices,nw);
...
if (err) < 1e-10
     fprintf('--- unittest_AR1prior_circ_nonunif: PASSED ---\n');
 else
     warning('unittest_AR1prior_circ_nonunif: FAILED');
 end
